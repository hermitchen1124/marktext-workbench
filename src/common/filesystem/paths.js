...
export const SUPPORTED_TEXT_INCLUSIONS = Object.freeze(SUPPORTED_TEXT_EXTENSIONS.map(x => '*.' + x))
...
/**
 * Returns true if the filename matches one of supported plain text extensions.
 *
 * @param {string} filename Path or filename
 */
export const hasSupportedTextExtension = filename => {
  if (!filename || typeof filename !== 'string') return false
...
/**
 * Returns true if the path is a supported plain text file or symbolic link to one.
 *
 * @param {string} filepath The path or link path.
 */
export const isSupportedTextFile = filepath => {
  if (!isFile2(filepath)) return false

  // Check symbolic link.
  if (isSymbolicLink(filepath)) {
    const targetPath = path.resolve(path.dirname(filepath), fs.readlinkSync(filepath))
    return isFile(targetPath) && hasSupportedTextExtension(targetPath)
  }
  return hasSupportedTextExtension(filepath)
}
