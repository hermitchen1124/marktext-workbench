...
class App {
  ...
  _openPathList (pathsToOpen, openFilesInSameWindow = false) {
    ...
    for (const window of _windowManager.windows.values()) {
      if (window.type === WindowType.EDITOR) {
        const openedDirs = Array.isArray(window.openedRootDirectories) && window.openedRootDirectories.length
          ? window.openedRootDirectories
          : [window.openedRootDirectory]
        for (const openedDir of openedDirs) {
          if (openedDir && directorySet.has(openedDir)) {
            window.bringToFront()
            directorySet.delete(openedDir)
          }
        }
      }
    }
  ...
  _listenForIpcMain () {
    ...
    ipcMain.on('app-open-directory-by-id', (windowId, pathname, openInSameWindow, keepExisting = false) => {
      const { openFolderInNewWindow } = this._accessor.preferences.getAll()
      if (openInSameWindow || !openFolderInNewWindow) {
        const editor = this._windowManager.get(windowId)
        if (editor) {
          editor.openFolder(pathname, keepExisting)
          return
        }
      }
    ...
    ipcMain.on('mt::remove-opened-directory', (e, pathname) => {
