...
import { hasMarkdownExtension, hasSupportedTextExtension } from 'common/filesystem/paths'
...
const splitSegments = pathname => pathname.split(/[\\/]+/).filter(Boolean)

const hasIgnoredNameSegment = segment => {
  if (!segment) return false
  if (segment.startsWith('.')) return true
  if (segment === 'node_modules') return true
  if (segment.endsWith('.asar')) return true
  return false
}

// Ignore hidden/system segments only in the path relative to the watched root.
// This allows explicitly opened roots like ".openclaw" to work as expected.
const shouldIgnoreByRelativePath = (watchRoot, pathname) => {
  if (!watchRoot || !pathname) return false

  const relativePath = path.relative(watchRoot, pathname)
  if (!relativePath || relativePath === '.') {
    return false
  }

  // Safety fallback for unexpected paths outside of watch root.
  if (relativePath.startsWith('..') || path.isAbsolute(relativePath)) {
    return splitSegments(pathname).some(hasIgnoredNameSegment)
  }

  return splitSegments(relativePath).some(hasIgnoredNameSegment)
}
...
const add = async (win, pathname, type, endOfLine, autoGuessEncoding, trimTrailingNewline) => {
  ...
  const isSupportedText = hasSupportedTextExtension(pathname)
  ...
  watch (win, watchPath, type = 'dir'/* file or dir */) {
    ...
    const watcher = chokidar.watch(watchPath, {
      ignored: (pathname, fileInfo) => {
        ...
        if (!fileInfo) {
          return shouldIgnoreByRelativePath(watchPath, pathname)
        }

        if (shouldIgnoreByRelativePath(watchPath, pathname)) {
          return true
        }
        if (fileInfo.isDirectory()) {
          return false
        }
        return !hasSupportedTextExtension(pathname)
      },
