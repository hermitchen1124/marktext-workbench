...
import { SUPPORTED_TEXT_EXTENSIONS, isSupportedTextFile } from 'common/filesystem/paths'
...
const handleResponseForSave = async (e, {
  id,
  filename,
  markdown,
  pathname,
  ...
  if (showSaveConfirm) {
    const { response } = await dialog.showMessageBox(win, {
      ...
      cancelId: 1,
      noLink: true,
      message: 'Confirm file modification?',
      detail: `Save changes to "${path.basename(filePath)}".`
    })
    ...
    .then(() => {
      ...
      if (notifyOnSave) {
        win.webContents.send('mt::show-notification', {
          title: 'Saved',
          type: 'primary',
          message: `"${path.basename(filePath)}" was saved successfully.`
        })
      }
      return id
    })
...
ipcMain.on('mt::response-file-save-as', async (e, {
  id,
  filename,
  markdown,
  pathname,
...
ipcMain.on('mt::ask-for-open-project-in-sidebar', async e => {
  ...
  if (Array.isArray(filePaths) && filePaths.length > 0) {
    for (const selectedPath of filePaths) {
      const resolvedPath = normalizeAndResolvePath(selectedPath)
      ipcMain.emit('app-open-directory-by-id', win.id, resolvedPath, true, true)
    }
  }
})
...
ipcMain.on('mt::format-link-click', (e, { data, dirname }) => {
  ...
  if (pathname) {
    ...
    if (isSupportedTextFile(pathname)) {
      const win = BrowserWindow.fromWebContents(e.sender)
...
ipcMain.on('mt::ask-for-import-files-in-sidebar', async e => {
  const win = BrowserWindow.fromWebContents(e.sender)
  const { filePaths } = await dialog.showOpenDialog(win, {
  ...
  const supportedFiles = filePaths
    ...
    .filter(filePath => isSupportedTextFile(filePath))
  ...
  const directories = Array.from(new Set(
    supportedFiles.map(filePath => path.dirname(filePath))
  ))
  for (const dirname of directories) {
    ipcMain.emit('app-open-directory-by-id', win.id, dirname, true, true)
  }
  ipcMain.emit('app-open-files-by-id', win.id, supportedFiles)
})

ipcMain.on('mt::ask-for-create-default-file-in-sidebar', async (e, defaultDir = '') => {
  const win = BrowserWindow.fromWebContents(e.sender)
  try {
    const dirname = defaultDir || getPath('documents')
    ...
    ipcMain.emit('app-open-directory-by-id', win.id, dirname, true, true)
    ipcMain.emit('app-open-file-by-id', win.id, filePath)
  } catch (err) {
    log.error('Error while creating default file:', err)
    win.webContents.send('mt::show-notification', {
...
ipcMain.on('mt::ask-for-create-file-manually-in-sidebar', async (e, defaultDir = '') => {
  const win = BrowserWindow.fromWebContents(e.sender)
  try {
    const safeDefaultDir = defaultDir || getPath('documents')
    await fs.ensureDir(safeDefaultDir)
    const { filePath, canceled } = await dialog.showSaveDialog(win, {
      defaultPath: path.join(safeDefaultDir, 'untitled.md'),
      filters: [{
        name: 'Supported text files',
        extensions: SUPPORTED_TEXT_EXTENSIONS
      }]
    ...
    const finalPath = extension ? filePath : `${filePath}.md`
    ...
    ipcMain.emit('app-open-directory-by-id', win.id, finalDir, true, true)
    ipcMain.emit('app-open-file-by-id', win.id, finalPath)
  } catch (err) {
    log.error('Error while creating file manually:', err)
    win.webContents.send('mt::show-notification', {
      title: 'Create file failure',
      type: 'error',
      message: err.message
