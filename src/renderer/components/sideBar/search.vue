    ...
    >
      <div class="search-wrapper">
        <input
          ...
          placeholder="Search in folder or opened files..."
          @keyup="search"
        >
        <div class="controls">
          <span
      ...
      <div class="search-message-section" v-if="showNoFolderOpenedMessage">
        <span>No searchable folder. Open a folder or import files first.</span>
      </div>
...
import { SUPPORTED_TEXT_INCLUSIONS } from '../../../common/filesystem/paths'
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      projectTrees: state => state.project.projectTrees,
    ...
    searchDirectories () {
      ...
      const directories = this.tabs
        .map(tab => tab && tab.pathname ? path.dirname(tab.pathname) : '')
        .filter(Boolean)

      return Array.from(new Set(directories))
    },
    showNoFolderOpenedMessage () {
      return this.searchDirectories.length === 0
    },
  ...
  methods: {
    search () {
      ...
      const promises = ripgrepDirectorySearcher.search(searchDirectories, keyword, {
        ...
        // Search all supported text files in sidebar workflow.
        inclusions: SUPPORTED_TEXT_INCLUSIONS
      })
        .then(() => {
