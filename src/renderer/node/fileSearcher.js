...
const getFriendlySpawnError = (error, rgPath) => {
  if (!error) {
    return new Error('File search failed to start ripgrep.')
  }
  if (error.code === 'ENOENT') {
    return new Error(`Ripgrep executable was not found ("${rgPath}"). Install ripgrep or set MARKTEXT_RIPGREP_PATH.`)
  }
  return error
}
...
class FileSearcher extends RipgrepDirectorySearcher {
  searchInDirectory (directoryPath, pattern, options, numPathsFound) {
    ...
    const returnedPromise = new Promise((resolve, reject) => {
      ...
      child.on('error', err => {
        reject(getFriendlySpawnError(err, this.rgPath))
      })
